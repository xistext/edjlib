...
Port to Pascal: Erik Donovan Johnson erikquiet@gmail.com
...
function PREV_INDEX(i, n : integer) : integer; begin result := ((i - 1 + n) mod n); end;
function NEXT_INDEX(i, n : integer) : integer; begin result := ((i + 1) mod n); end;
...
begin
  { edge := nil; never used according to compiler }
  ...
  begin
    ...
    result_polygon^.contour := nil;
    reset_lmt(lmt);
    IF ASSIGNED( s_heap ) THEN
       FREE(pointer(s_heap));
    IF ASSIGNED( c_heap ) THEN
       FREE(pointer(c_heap));
    exit;
  end;
  ...
  begin
    ...
    e0 := aet;
//  e1 := aet; never used according to compiler }
